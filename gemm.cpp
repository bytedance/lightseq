...
This is the sample code to profile :
    ...
    2. INT8 NT-gemm + cublasLt + INT8 Output. (using tensor core + IMMA specific layout) (using alpha to quantize)
    3. INT8 TN-gemm + cublasLt + INT8 Output. (using tensor core + col-major layout) (using alpha to quantize)
...
template<typename T>
void matMul(int m, int n, int k, const T* A, int lda, const T* B, int ldb, float* C, int ldc)
{
    float sum;
    for (int c = 0; c < n; c++) {
        // printf("CPU gemm : %f\n", float(c)/float(n));
...
#define checkcuBlasError(F)                                                                                            \
    if ((F) != CUBLAS_STATUS_SUCCESS) {                                                                                \
        printf("Error at line %d in file %s: %s\n", __LINE__, __FILE__, _cudaGetErrorEnum(F));                         \
        exit(-1);                                                                                                      \
    }
...
void checkMat2(TA* A, TB* B, int size, string mark)
{
    ...
    float B_nonZero_ratio = float(checkNonZero(B, size)) / float(size);
    if (A_nonZero_ratio < 0.1 || B_nonZero_ratio < 0.1)
        printf("[checkMat2 %s] nonZero ratio [%f] [%f]\n", mark.c_str(), A_nonZero_ratio, B_nonZero_ratio);
    int idx = 0;
    for (int jjj = 0; jjj < size; jjj++) {
        if (fabs(float(matA[jjj]) - float(matB[jjj])) > 0.00001) {
            not_passed += 1;
            if (idx < 1000) {
                printf("%d %f %f %f\n", idx, float(matA[jjj]), float(matB[jjj]), float(matA[jjj]) - float(matB[jjj]));
                idx += 1;
            }
        }
    }
...
void checkMat3(TA* A, TB* B, int size, float alpha, string mark)
{
    ...
    int idx = 0;
    for (int jjj = 0; jjj < size; jjj++) {
        if (fabs(float(round(float(matA[jjj]) * alpha)) - float(matB[jjj])) > 0.00001) {
            not_passed += 1;
            if (idx < 1000) {
                printf("%d %f %f %f\n",
                       idx,
                       float(matA[jjj]) * alpha,
                       float(matB[jjj]),
...
int LtIgemmCustomFindINT8OutputCOL32_2R_4R4(cublasLtHandle_t ltHandle,
                                            int m,
                                            int n,
                                            int k,
...
#define ALGO_IDS 100
    ...
    cudaDataType_t scaleType = CUDA_R_32F, Atype = CUDA_R_8I, Btype = CUDA_R_8I, Ctype = CUDA_R_8I;
    ...
    if (RUN_WITH_A100 == 0)
        ...
        cublasLtMatrixLayoutSetAttribute(
            BtransformDesc, CUBLASLT_MATRIX_LAYOUT_ORDER, &order_COL32_2R_4R4, sizeof(order_COL32_2R_4R4));
    ...
    cublasLtMatmulDescSetAttribute(operationDesc, CUBLASLT_MATMUL_DESC_SCALE_TYPE, &scaleType, sizeof(scaleType));
    ...
    for (int i = 0; i < AlgoCount; i++) {
        printf("INT8 NT-gemm ORDER_COL32_2R_4R4 INT8 IO cublasLt %03d : ", i);
...
void int8gemm_with_cublasLtMatmul_ORDER_COL32_2R_4R4(int m,
                                                     int n,
                                                     int k,
                                                     float alpha2,
                                                     float beta2,
                                                     ...
                                                     cudaStream_t stream)
{
    ...
    cudaMalloc(&Atransform, sizeof(int8_t) * roundoff(k, 32) / 32 * ldaTransform);
    cudaMalloc(&Btransform, sizeof(int8_t) * roundoff(k, 32) / 32 * ldbTransform);
    cudaMalloc(&Ctransform2, sizeof(int8_t) * roundoff(n, 32) / 32 * ldcTransform);
    ...
    cublasLtMatrixLayoutSetAttribute(CtransformDesc2, CUBLASLT_MATRIX_LAYOUT_ORDER, &order_COL32, sizeof(order_COL32));
    ...
    cublasLtMatmulDescSetAttribute(
        operationDesc2, CUBLASLT_MATMUL_DESC_TRANSB, &opTranspose, sizeof(cublasOperation_t));
    // using alpha to quantize
    cublasLtMatmulDescSetAttribute(operationDesc2, CUBLASLT_MATMUL_DESC_SCALE_TYPE, &scaleType, sizeof(scaleType));
    ...
    cublasLtMatmulAlgoInit(ltHandle,
                           ...
                           CUDA_R_8I,
                           CUDA_R_8I,
                           INT8_gemm_cublasLt_INT8Output_best_algo.algoId,
                           &algo2);
    ...
    cublasLtMatmulAlgoConfigSetAttribute(&algo2,
                                         CUBLASLT_ALGO_CONFIG_REDUCTION_SCHEME,
                                         &(INT8_gemm_cublasLt_INT8Output_best_algo.reductionScheme),
                                         sizeof(int));
    cublasLtMatmulAlgoConfigSetAttribute(&algo2,
    ...
    if (INT8_gemm_cublasLt_INT8Output_best_algo.workspaceSize != 0) {
        cudaMalloc((void**)&workSpace2, sizeof(char) * INT8_gemm_cublasLt_INT8Output_best_algo.workspaceSize);
    }

    cublasStat = cublasLtMatrixTransform(ltHandle,
                                         transformDesc,
                                         &transformAlpha,
                                         d_int8_A,
                                         Adesc,
    ...
    for (int t = 0; t < iters; t++) {
        cublasStat = cublasLtMatmul(ltHandle,
                                    operationDesc2,
                                    &alpha2,
                                    Atransform,
                                    AtransformDesc,
                                    ...
                                    Ctransform2,
                                    CtransformDesc2,
                                    // NULL, workSpace2, workspaceSize, stream);
                                    &algo2,
                                    workSpace2,
                                    workspaceSize,
                                    stream);
    ...
    if (cublasStat == CUBLAS_STATUS_SUCCESS) {
        printf("INT8 NT-gemm with B = ORDER_COL32_2R_4R4 cublasLtMatmul INT8 output best algo %d exec_time %f(ms)\n",
               INT8_gemm_cublasLt_INT8Output_best_algo.algoId,
               time_used);
    }
    ...
    if (Adesc)
        cublasLtMatrixLayoutDestroy(Adesc);
    if (CtransformDesc2)
        cublasLtMatrixLayoutDestroy(CtransformDesc2);
    if (BtransformDesc)
        cublasLtMatrixLayoutDestroy(BtransformDesc);
    if (AtransformDesc)
        cublasLtMatrixLayoutDestroy(AtransformDesc);
    if (operationDesc2)
        cublasLtMatmulDescDestroy(operationDesc2);
    if (transformDesc)
        cublasLtMatrixTransformDescDestroy(transformDesc);
    if (Atransform)
        cudaFree(Atransform);
...
// find the best algo for INT8 gemm + cublasLt + INT8 Output
int LtIgemmCustomFindINT8OutputColMajor(cublasLtHandle_t ltHandle,
                                        int m,
                                        int n,
                                        int k,
                                        const float* alpha, /* host pointer */
...
#define ALGO_IDS 100
    ...
    cublasLtMatrixLayoutSetAttribute(Adesc, CUBLASLT_MATRIX_LAYOUT_ORDER, &order_col_major, sizeof(order_col_major));
    cublasLtMatrixLayoutCreate(&Bdesc, CUDA_R_8I, k, n, k);
    cublasLtMatrixLayoutSetAttribute(Bdesc, CUBLASLT_MATRIX_LAYOUT_ORDER, &order_col_major, sizeof(order_col_major));
    cublasLtMatrixLayoutCreate(&Cdesc, CUDA_R_8I, m, n, m);
    cublasLtMatrixLayoutSetAttribute(Cdesc, CUBLASLT_MATRIX_LAYOUT_ORDER, &order_col_major, sizeof(order_col_major));

    cublasLtMatmulDescCreate(&operationDesc, computeType, scaleType);
    // Tensor op igemm kernels only support TN gemm
    cublasLtMatmulDescSetAttribute(operationDesc, CUBLASLT_MATMUL_DESC_TRANSA, &opTranspose, sizeof(cublasOperation_t));
    // using alpha to quantize
    cublasLtMatmulDescSetAttribute(operationDesc, CUBLASLT_MATMUL_DESC_SCALE_TYPE, &scaleType, sizeof(scaleType));
...
void int8gemm_with_cublasLtMatmul_COL_MAJOR(int m,
                                            int n,
                                            int k,
                                            float alpha2,
                                            float beta2,
                                            ...
                                            cudaStream_t stream)
{
    ...
    cublasLtMatrixLayoutSetAttribute(Bdesc, CUBLASLT_MATRIX_LAYOUT_ORDER, &order_COL_MAJOR, sizeof(order_COL_MAJOR));
    cublasLtMatrixLayoutCreate(&Cdesc2, CUDA_R_8I, m, n, m);
    cublasLtMatrixLayoutSetAttribute(Cdesc2, CUBLASLT_MATRIX_LAYOUT_ORDER, &order_COL_MAJOR, sizeof(order_COL_MAJOR));

    cublasLtMatmulDescCreate(&operationDesc2, computeType, scaleType);
    // Tensor op igemm kernels only support NT gemm
    cublasLtMatmulDescSetAttribute(
        operationDesc2, CUBLASLT_MATMUL_DESC_TRANSA, &opTranspose, sizeof(cublasOperation_t));
    // using alpha to quantize
    cublasLtMatmulDescSetAttribute(operationDesc2, CUBLASLT_MATMUL_DESC_SCALE_TYPE, &scaleType, sizeof(scaleType));
    ...
    time_used /= iters;
    if (cublasStat == CUBLAS_STATUS_SUCCESS) {
        printf("INT8 TN-gemm with B = ORDER_COL_MAJOR cublasLtMatmul INT8 output best algo %d exec_time %f(ms)\n",
               INT8_gemm_cublasLt_INT8Output_best_algo.algoId,
               time_used);
    }
...
int main(int argc, char** argv)
{
    ...
    checkCudaAPIErrors(cudaMalloc((void**)&d_half_A_transpose, sizeof(half) * m * k));
    checkCudaAPIErrors(cudaMalloc((void**)&d_half_B_transpose, sizeof(half) * n * k));
    ...
    checkCudaAPIErrors(cudaMalloc((void**)&d_int8_A_transpose, sizeof(int8_t) * m * k));
    checkCudaAPIErrors(cudaMalloc((void**)&d_int8_B_transpose, sizeof(int8_t) * n * k));
    ...
    checkCudaAPIErrors(cudaMalloc((void**)&d_int8_C_col_major, sizeof(int8_t) * m * n));
    ...
    cudaMemcpy(d_int8_A, h_int8_A, m * k * sizeof(int8_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_int8_B, h_int8_B, n * k * sizeof(int8_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_int8_A_transpose, h_int8_A_transpose, m * k * sizeof(int8_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_int8_B_transpose, h_int8_B_transpose, n * k * sizeof(int8_t), cudaMemcpyHostToDevice);
    cudaMemcpy(d_half_A_transpose, h_half_A_transpose, m * k * sizeof(half), cudaMemcpyHostToDevice);
    cudaMemcpy(d_half_B_transpose, h_half_B_transpose, n * k * sizeof(half), cudaMemcpyHostToDevice);

    cublasLtMatmulAlgo_info INT8_gemm_cublasLt_INT8Output_best_algo, INT8_gemm_cublasLt_INT8Output_COLMajor_best_algo;
    ...
    float alpha2 = 1.0f;
    float beta2 = 0.f;
    printf("#########find the best algorithms for int8 NT gemm with COL32_2R_4R4#########\n");
    LtIgemmCustomFindINT8OutputCOL32_2R_4R4(
        ltHandle, n, m, k, &alpha2, d_int8_B, d_int8_A, &beta2, d_int8_C, INT8_gemm_cublasLt_INT8Output_best_algo);

    // step 2.2 : find the best algorithms for int8 TN gemm with COLMajor
    printf("#########find the best algorithms for int8 TN gemm with COLMajor#########\n");
    LtIgemmCustomFindINT8OutputColMajor(ltHandle,
                                        n,
                                        m,
                                        k,
                                        &alpha2,
    ...
    int iters = 100;
    printf("#########Profile using best algorithms#########\n");
    cudaEventRecord(start, 0);
    for (int t = 0; t < iters; t++) {
    ...
    // step 3.2 : INT8 + gemm + COL32_2R_4R4 + cublasLt + INT8 output (using tensor core)
    alpha2 = 127.0f / getAMax(d_half_C, m * n);
    int8gemm_with_cublasLtMatmul_ORDER_COL32_2R_4R4(n,
                                                    m,
                                                    k,
                                                    alpha2,
                                                    beta2,
    ...
    // step 3.3 : INT8 + gemm + COL-MAJOR + cublasLt + INT8 output (using tensor core)
    alpha2 = 127.0f / getAMax(d_half_C, m * n);
    int8gemm_with_cublasLtMatmul_COL_MAJOR(n,
                                           m,
                                           k,
                                           alpha2,
                                           beta2,
    ...
    // checkMat3(h_float_C, d_int8_C_col_major, m*n, alpha2, "d_int8_C_col_major");
    // step 5: free memory
    free(h_half_A);
