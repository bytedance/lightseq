    ...
    def inject_origin_layer(self, hf_layer, ls_layer, is_decoder=False):
        if not is_decoder:
            for layer_id in range(self.config.encoder_layers):
                ...
                layer.self_attn_layer_norm.weight.data.copy_(weight["self_attn_layer_norm"])
            ...
            for layer_id in range(self.config.decoder_layers):
                ...
                layer.encoder_attn.q_proj.weight.data.copy_(weight["encoder_attn_q_proj"])
                layer.encoder_attn.q_proj.bias.data.copy_(bias["encoder_attn_q_proj"])
                layer.encoder_attn.out_proj.weight.data.copy_(weight["encoder_attn_out_proj"])
                layer.encoder_attn.out_proj.bias.data.copy_(bias["encoder_attn_out_proj"])
                layer.encoder_attn_layer_norm.weight.data.copy_(weight["encoder_attn_layer_norm"])
                layer.encoder_attn_layer_norm.bias.data.copy_(bias["encoder_attn_layer_norm"])
                if layer_id == 0:
                ...
                layer.encoder_attn.k_proj.weight.data.copy_(encoder_attn_k_proj_w[layer_id])
                layer.encoder_attn.k_proj.bias.data.copy_(encoder_attn_k_proj_b[layer_id])
