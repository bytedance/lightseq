...
def load_huggingface_model(state_dict, config):
...
@torch.no_grad()
def get_ls_state_dict(checkpoint_path, hf_checkpoint_name):
    device = "cuda:0"
    dirname, modelname = split_checkpoint_path(checkpoint_path)

    hf_config = BartConfig.from_pretrained(hf_checkpoint_name)
    # del state_dict['args'].gpt2_encoder_json
    # del state_dict['args'].gpt2_vocab_bpe
    ls_model = (
        LSBARTModel.from_pretrained(dirname, 
            checkpoint_file=modelname, 
            fp16=False,
            gpt2_encoder_json=DEFAULT_ENCODER_JSON,
            gpt2_vocab_bpe=DEFAULT_VOCAB_BPE,
            )
        .to(device)
        .eval()
    )
    ...
    # return state_dict
    # load huggface model
    hf_model = load_huggingface_model(state_dict, hf_config).to(
        device
    )
...
if __name__ == "__main__":
    ...
    checkpoint_path = '/opt/tiger/begin/lightseqx/bart/writer/checkpoint2.pt'
    hf_config_name = 'facebook/bart-base'

    head_number = 12  # change this to 16 for "bart-large" model
    beam_size = 4
    max_step = 200  # max step for generation, it decides GPU memory occupancy
    # maximum_generation_length = min(src_length + extra_decode_length, max_step)
    extra_decode_length = 50
    length_penalty = 1.0
    bart_model_state_dict = get_ls_state_dict(checkpoint_path, hf_config_name)
    extract_transformer_weights(
        output_lightseq_model_name,
        bart_model_state_dict,
    ...
    inputs = torch.tensor([[4993, 50260, 250, 506, 90, 242, 338, 
        50260, 14, 50260, 90, 298, 102, 90, 50260, 2156, 50260, 6, 
        50260, 38, 50260, 100, 50260, 889, 1397, 50260, 462, 118, 
        29, 90, 242, 282, 417, 50260, 7, 50260, 90, 139, 50260, 930,
        50260, 119, 257, 29, 118, 438, 50260, 479, 50260, 4,
        50260, 2]])

    ls_model = lsi.Transformer(f"{output_lightseq_model_name}.hdf5", 128)
    generated_ids = ls_model.infer(inputs)
    print(generated_ids)
