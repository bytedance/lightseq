...
def ls_generate(model, tokenizer, inputs_id):
    ...
    ls_res_ids, ls_time = ls_mt5(model, inputs_id)
...
def hf_generate(model, tokenizer, inputs_id):
    ...
    hf_res_ids, hf_time = hf_mt5(model, inputs_id)
...
def main():
    ...
    print("initializing mt5 tokenizer...")
    tokenizer = MT5Tokenizer.from_pretrained("google/mt5-base")

    print("creating lightseq model...")
    ls_model = lsi.MT5("lightseq_mt5_base.hdf5", 128)

    print("creating huggingface model...")
    hf_model = MT5ForConditionalGeneration.from_pretrained("google/mt5-base")
    hf_model.to("cuda:0")
    hf_model.eval()
