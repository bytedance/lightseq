...
DeltalmDecoder<OpType_>::DeltalmDecoder(int max_batch_size, const int* p_d_padding_mask,
                          const _DataType* p_d_encoder_output, int* p_d_result,
                          DeltalmWeight<OpType_>& tw, cudaStream_t stream,
...
*/
template <OperationType OpType_>
long DeltalmDecoder<OpType_>::compute_buffer_bytesize() {
...
*/
template <OperationType OpType_>
void DeltalmDecoder<OpType_>::init_buffer(void* pbuf) {
...
*/
template <OperationType OpType_>
std::string DeltalmDecoder<OpType_>::check() {
...
*/
template <OperationType OpType_>
void DeltalmDecoder<OpType_>::run_one_infer(int batch_size, int batch_seq_len) {
...
*/
template <OperationType OpType_>
void DeltalmDecoder<OpType_>::project_encoder_output() {
...
*/
template <OperationType OpType_>
bool DeltalmDecoder<OpType_>::run_step() {
  embedding();
  decoder_stack();
...
*/
template <OperationType OpType_>
void DeltalmDecoder<OpType_>::embedding() {
...
*/
template <OperationType OpType_>
void DeltalmDecoder<OpType_>::decoder_stack() {
...
*/
template <OperationType OpType_>
void DeltalmDecoder<OpType_>::self_attention() {
...
*/
template <OperationType OpType_>
void DeltalmDecoder<OpType_>::encdec_attention() {
  /* ---step 0. layer_norm, add output_bias to "query"--- */
  ker_norm_layer_resual_launcher<_DataType>(
      _step_token_num, _tw._hidden_size, _stream, _p_d_cur_step_query,
...
template <OperationType OpType_>
void DeltalmDecoder<OpType_>::ffn_add_norm() {
  /* ---step 0. layer_norm, add output_bias to "query"--- */
  ker_norm_layer_resual_launcher<_DataType>(
      _step_token_num, _tw._hidden_size, _stream, _p_d_cur_step_query,
...
template <OperationType OpType_>
void DeltalmDecoder<OpType_>::middle_ffn_add_norm() {
  /* ---step 0. layer_norm, add output_bias to "query"--- */
  ker_norm_layer_resual_launcher<_DataType>(
      _step_token_num, _tw._hidden_size, _stream, _p_d_cur_step_query,
      _p_d_query_buf1, _p_d_dec_wei[_weight_offset + 12 + 6],
      _p_d_dec_wei[_weight_offset + 13 + 6], _p_d_dec_wei[_weight_offset + 17 + 6],
      _max_thread_per_block, _tw._is_post_ln);

#ifdef DEBUG_RESULT
  print_vec(_p_d_query_buf1, "middle ffn ln(head): ", 5);
  print_vec(_p_d_query_buf1 + _step_token_num * _tw._hidden_size - 5,
            "middle ffn ln(tail): ", 5);
#endif

  /* ---step 1. first ffn layer--- */
  CHECK_GPU_ERROR(cublasGemmEx(
      _hd, CUBLAS_OP_N, CUBLAS_OP_N, _tw._inner_size, _step_token_num,
      _tw._hidden_size, &_type_one, _p_d_dec_wei[_weight_offset + 14 + 6], _AType,
      _tw._inner_size, _p_d_query_buf1, _BType, _tw._hidden_size, &_type_zero,
      _p_d_query_buf2, _CType, _tw._inner_size, _computeType,
      CUBLAS_GEMM_DEFAULT_TENSOR_OP));
  ...
  if (_tw._use_gelu) {
    ker_bias_gelu_launcher<_DataType>(
        _step_token_num, _max_thread_per_block, _stream, _p_d_query_buf2,
        _p_d_dec_wei[_weight_offset + 15 + 6], _tw._inner_size);
  } else {
    ker_bias_relu_launcher<_DataType>(
        _step_token_num, _max_thread_per_block, _stream, _p_d_query_buf2,
        _p_d_dec_wei[_weight_offset + 15 + 6], _tw._inner_size);
  ...
  CHECK_GPU_ERROR(cublasGemmEx(
      _hd, CUBLAS_OP_N, CUBLAS_OP_N, _tw._hidden_size, _step_token_num,
      _tw._inner_size, &_type_one, _p_d_dec_wei[_weight_offset + 16 + 6], _AType,
      _tw._hidden_size, _p_d_query_buf2, _BType, _tw._inner_size, &_type_one,
      _p_d_cur_step_query, _CType, _tw._hidden_size, _computeType,
      CUBLAS_GEMM_DEFAULT_TENSOR_OP));
  return;
...
template <OperationType OpType_>
bool DeltalmDecoder<OpType_>::sample() {
  CHECK_GPU_ERROR(
...
template <OperationType OpType_>
bool DeltalmDecoder<OpType_>::beam_search() {
  /*
...
*/
template <OperationType OpType_>
void DeltalmDecoder<OpType_>::update_new_seq_probs() {
  CHECK_GPU_ERROR(cudaMemsetAsync(_p_d_can_num, 0, sizeof(int), _stream));
...
template <OperationType OpType_>
bool DeltalmDecoder<OpType_>::topk_greedy_search() {
...
template class DeltalmDecoder<OperationType::FP16>;
template class DeltalmDecoder<OperationType::FP32>;
