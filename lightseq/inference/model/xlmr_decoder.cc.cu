...
#include "xlmr_decoder.h"
#include "../kernels/transformerKernels.h"
#include "../kernels/XLMRKernels.h"

/**
@file
XLMR decoder, composed by gemm lib and
...
XLMRDecoder<OpType_>::XLMRDecoder(int max_batch_size,
                                  const int* p_d_padding_mask,
                                  const int* p_d_lang_id,
                                  const _DataType* p_d_encoder_output,
                                  int* p_d_result, XLMRWeight<OpType_>& tw,
                                  cudaStream_t stream, cublasHandle_t hd,
                                  bool output_topk, const int* p_d_token_id)
...
/**
Compute GPU memory size needed by xlmr decoder,
  to see how these memory is used, checkout init_buffer() for detail
*/
template <OperationType OpType_>
long XLMRDecoder<OpType_>::compute_buffer_bytesize() {
...
*/
template <OperationType OpType_>
void XLMRDecoder<OpType_>::init_buffer(void* pbuf) {
...
*/
template <OperationType OpType_>
std::string XLMRDecoder<OpType_>::check() {
...
/**
XLMRDecoder inference
*/
template <OperationType OpType_>
void XLMRDecoder<OpType_>::run_one_infer(int batch_size, int batch_seq_len) {
  /* ---step1. init--- */
...
*/
template <OperationType OpType_>
void XLMRDecoder<OpType_>::project_encoder_output() {
...
*/
template <OperationType OpType_>
bool XLMRDecoder<OpType_>::run_step() {
  embedding();
  decoder_stack();
...
/**
XLMRDecoder feedforward, composed by self_atten,
  enc-dec-atten, ffn
*/
template <OperationType OpType_>
void XLMRDecoder<OpType_>::decoder_stack() {
...
/**
XLMRDecoder self attention
*/
template <OperationType OpType_>
void XLMRDecoder<OpType_>::self_attention() {
...
/**
Encode-XLMRDecoder attention
*/
template <OperationType OpType_>
void XLMRDecoder<OpType_>::encdec_attention() {
  /* ---step 0. layer_norm, add output_bias to "query"--- */
  ker_norm_layer_resual_launcher<_DataType>(
      _step_token_num, _tw._hidden_size, _stream, _p_d_cur_step_query,
...
template <OperationType OpType_>
void XLMRDecoder<OpType_>::ffn_add_norm() {
  /* ---step 0. layer_norm, add output_bias to "query"--- */
  ker_norm_layer_resual_launcher<_DataType>(
      _step_token_num, _tw._hidden_size, _stream, _p_d_cur_step_query,
...
template <OperationType OpType_>
bool XLMRDecoder<OpType_>::sample() {
  CHECK_GPU_ERROR(
...
template <OperationType OpType_>
bool XLMRDecoder<OpType_>::beam_search() {
  /*
...
*/
template <OperationType OpType_>
void XLMRDecoder<OpType_>::update_new_seq_probs() {
  CHECK_GPU_ERROR(cudaMemsetAsync(_p_d_can_num, 0, sizeof(int), _stream));
...
template <OperationType OpType_>
bool XLMRDecoder<OpType_>::topk_greedy_search() {
...
template class XLMRDecoder<OperationType::FP16>;
template class XLMRDecoder<OperationType::FP32>;
