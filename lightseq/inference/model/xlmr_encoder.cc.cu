...
XLMREncoder<OpType_>::XLMREncoder(int max_batch_size, const int *p_d_token_id,
                                  const int *p_d_lang_id, int *p_d_padding_mask,
                                  _DataType *p_d_output,
                                  const XLMRWeight<OpType_> &tw,
...
/**
Compute GPU memory size needed by xlmr encoder,
  to see how these memory is used, checkout init_buffer() for detail
*/
template <OperationType OpType_>
long XLMREncoder<OpType_>::compute_buffer_bytesize() {
...
*/
template <OperationType OpType_>
void XLMREncoder<OpType_>::init_buffer(void *pbuf) {
...
*/
template <OperationType OpType_>
std::string XLMREncoder<OpType_>::check() {
...
/**
XLMREncoder inference
*/
template <OperationType OpType_>
void XLMREncoder<OpType_>::run_one_infer(int batch_size, int batch_seq_len) {
  /* ---step1. init--- */
  ...
  ker_xlmr_enc_emb_launcher(
      batch_size, batch_seq_len, _tw._hidden_size, _stream, _p_d_src_emb_wei[0],
      _p_d_src_emb_wei[1], _p_d_src_emb_wei[4], _p_d_token_id, _p_d_lang_id,
      _p_d_output, _p_d_padding_mask, _tw._padding_id, _max_thread_per_block);

#ifdef DEBUG_RESULT
  print_vec(_p_d_src_emb_wei[0] + _tw._hidden_size, "token emb", 10);
  print_vec(_p_d_src_emb_wei[1], "encoder pos emb", 10);
  print_vec(_p_d_src_emb_wei[4] + 21 * _tw._hidden_size, "encoder lang emb",
            10);
  print_vec(_p_d_lang_id, "encoder lang id", batch_size);
...
/**
XLMREncoder self attention
*/
template <OperationType OpType_>
void XLMREncoder<OpType_>::self_attention() {
...
template <OperationType OpType_>
void XLMREncoder<OpType_>::ffn_add_norm() {
  /* ---step 0. layer_norm, add output_bias to "query"--- */
  ker_norm_layer_resual_launcher<_DataType>(
...
#ifdef DEBUG_RESULT
  print_vec(_p_d_ffn_buf1, "ffn attn ln(head): ", 5);
...
template class XLMREncoder<OperationType::FP16>;
template class XLMREncoder<OperationType::FP32>;
