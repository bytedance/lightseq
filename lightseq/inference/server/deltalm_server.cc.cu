...
#include "../model/deltalm_decoder.h"
#include "../model/deltalm_encoder.h"
#include "model_config.pb.h"
#include "../proto/deltalm_weight.h"
...
class Context {
 ...
 private:
  ...
  lightseq::cuda::DeltalmWeight<OPTYPE> tw_;
  std::shared_ptr<lightseq::cuda::DeltalmDecoder<OPTYPE>> decoder_;
  std::shared_ptr<lightseq::cuda::DeltalmEncoder<OPTYPE>> encoder_;
};
...
int Context::Init() {
  ...
  std::string weight_path = model_path + "/" + "deltalm.pb";
  std::ifstream fproto(weight_path.c_str());
  if (!fproto.good()) {
    weight_path = model_path + "/" + "deltalm.hdf5";
    std::ifstream fhdf5(weight_path.c_str());
    if (!fhdf5.good()) {
      LOG_ERROR << "Neither deltalm.pb nor deltalm.hdf5 "
  ...
  encoder_ = std::make_shared<lightseq::cuda::DeltalmEncoder<OPTYPE>>(
  ...
  decoder_ = std::make_shared<lightseq::cuda::DeltalmDecoder<OPTYPE>>(
  ...
  cuerr = cudaStreamSynchronize(stream_);
  if (cuerr != cudaSuccess) {
    LOG_ERROR << "failed to init GPU for deltalm: "
              << cudaGetErrorString(cuerr) << std::endl;
    return kCudaExecute;
  }
  LOG_INFO << "Deltalm, release-version[" << __DATE__ << " " << __TIME__
...
int Context::GetInputTensorGPU(CustomGetNextInputFn_t input_fn,
  ...
  while (true) {
    ...
    if (cuerr != cudaSuccess) {
      LOG_ERROR << "failed to copy input values to GPU for deltalm: "
                << cudaGetErrorString(cuerr) << std::endl;
...
int Context::ExecuteGPU(const uint32_t payload_cnt, CustomPayload* payloads,
  ...
  for (uint32_t pidx = 0; pidx < payload_cnt; ++pidx) {
    ...
    if (cuerr != cudaSuccess) {
      LOG_ERROR << "failed to copy output values from GPU for deltalm: "
                << cudaGetErrorString(cuerr) << std::endl;
  ...
  if (cuerr != cudaSuccess) {
    LOG_ERROR << "failed to synchronize GPU for deltalm: "
              << cudaGetErrorString(cuerr) << std::endl;
    return kCudaExecute;
  }
  return kSuccess;
...
const char* CustomErrorString(void* custom_context, int errcode) {
  switch (errcode) {
    ...
    case kWeightLoad:
      return "load deltalm weight in .pb failed";
    case kModelSize:
      return "inappropriate deltalm model size";
    default:
      break;
  }
