...
Deltalm::Deltalm(const std::string weight_path,
  ...
  */
  ...
  encoder_ = std::make_shared<DeltalmEncoder<deltalm_optytpe>>(
  ...
  decoder_ = std::make_shared<DeltalmDecoder<deltalm_optytpe>>(
  ...
  std::cout << "Allocated " << buf_bytesize / (1024 * 1024)
            << "MB GPU buffer for deltalm" << std::endl;
...
const int *Deltalm::get_result_ptr() { return d_output_; }

const float *Deltalm::get_score_ptr() {
  return decoder_->_p_d_alive_seq_score;
}

int Deltalm::get_output_seq_len() { return decoder_->_cur_step + 1; };
...
void Deltalm::set_input_ptr(int index, void *input_ptr) {
  switch (index) {
    case 0:
...
void Deltalm::set_output_ptr(int index, void *output_ptr) {
  switch (index) {
    case 0:
...
const void *Deltalm::get_output_ptr(int index) {
  switch (index) {
    case 0:
...
std::vector<int> Deltalm::get_input_max_shape(int index) {
  switch (index) {
    case 0:
...
std::vector<int> Deltalm::get_output_max_shape(int index) {
  switch (index) {
    case 0:
...
DataType Deltalm::get_input_dtype(int index) {
  switch (index) {
    case 0:
...
DataType Deltalm::get_output_dtype(int index) {
  switch (index) {
    case 0:
      return DataType::kInt32;
      break;
