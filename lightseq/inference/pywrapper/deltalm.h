...
#ifdef FP16_MODE
const lightseq::cuda::OperationType deltalm_optytpe =
...
namespace lightseq {
namespace cuda {
class Deltalm : public LSModel {
 private:
  typedef OperationTypeTraits<deltalm_optytpe> optraits;
  std::shared_ptr<DeltalmEncoder<deltalm_optytpe>> encoder_;
  std::shared_ptr<DeltalmDecoder<deltalm_optytpe>> decoder_;
  ...
  cublasHandle_t hd_;
  DeltalmWeight<deltalm_optytpe> tw_;
 ...
 public:
  Deltalm(const std::string weight_path, const int max_batch_size);
  ~Deltalm();
