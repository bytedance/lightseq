...
class XLMRWeight {
 private:
  ...
  void proto_get_model_config(const XLMR &xlmr, bool only_decoder = false);
  std::string proto_parse_emb_wei(const XLMREmbeddingLayer &layer,
                                  std::string source);
  std::string proto_parse_enc_wei(const XLMR &xlmr);
  std::string proto_parse_dec_wei(const XLMR &xlmr);
