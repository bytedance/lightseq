...
class DeltalmWeight {
 private:
  ...
  void proto_get_model_config(const Deltalm &deltalm,
                              bool only_decoder = false);
  std::string proto_parse_emb_wei(const DeltalmEmbeddingLayer &layer,
                                  std::string source);
  std::string proto_parse_enc_wei(const Deltalm &deltalm);
  std::string proto_parse_dec_wei(const Deltalm &deltalm);
  ...
  std::vector<const _DataType *> _p_d_dec_wei;      // size: 24 * dec_layer_num
