...
void MT5Weight<OpType_>::proto_get_model_config(const MT5 &mt5,
  ...
  _inner_size = mt5.decoder_stack()[0].ffn_first_kernel_size() / _hidden_size;
  if (!only_decoder) {
    _src_vocab_size = mt5.src_embedding().token_embedding_size() / _hidden_size;
  }
  _trg_vocab_size = mt5.trg_embedding().token_embedding_size() / _hidden_size;

  if (!only_decoder) {
    _n_enc_layer = mt5.encoder_stack_size();
  }
  _n_dec_layer = mt5.decoder_stack_size();
  _head_num = mt5.model_conf().head_num();
  ...
  _beam_size = mt5.model_conf().beam_size();
  _extra_decode_length = mt5.model_conf().extra_decode_length();
  _length_penalty = mt5.model_conf().length_penalty();
  _padding_id = mt5.model_conf().src_padding_id();
  _start_id = mt5.model_conf().trg_start_id();
  _end_id = mt5.model_conf().trg_end_id();
  if (_end_id == 0) {
    _end_id = _trg_vocab_size - 1;
  }
  _diverse_lambda = mt5.model_conf().diverse_lambda();
  _sampling_method = mt5.model_conf().sampling_method();
  if (_sampling_method == "") {
    _sampling_method = "beam_search";
  }
  _topk = mt5.model_conf().topk();
  _topp = mt5.model_conf().topp();
  _is_post_ln = mt5.model_conf().is_post_ln();
  _no_scale_embedding = mt5.model_conf().no_scale_embedding();
  _use_gelu = mt5.model_conf().use_gelu();
  _multilg_type = mt5.model_conf().multilg_type();
}
...
*/
template <OperationType OpType_>
std::string MT5Weight<OpType_>::proto_parse_emb_wei(
    const MT5EmbeddingLayer &layer, std::string source) {
...
*/
template <OperationType OpType_>
std::string MT5Weight<OpType_>::proto_parse_enc_wei(const MT5 &mt5) {
  std::vector<int> offset;
  std::vector<float> value;
  int idx = 0;

  for (auto enc_layer : mt5.encoder_stack()) {
    offset.push_back(idx);
...
*/
template <OperationType OpType_>
std::string MT5Weight<OpType_>::proto_parse_dec_wei(const MT5 &mt5) {
  std::vector<int> offset;
  std::vector<float> value;
  int idx = 0;

  for (auto dec_layer : mt5.decoder_stack()) {
    offset.push_back(idx);
...
std::string MT5Weight<OpType_>::initializing(std::string weight_path,
  ...
  if (endswith(weight_path, ".pb")) {
    ...
    if (!mt5.ParseFromIstream(&raw_input)) {
      return "Parse weights from [" + weight_path + "] failed.";
    }
    proto_get_model_config(mt5, only_decoder);
    ...
    std::string res;
    if (!only_decoder) {
      res = proto_parse_emb_wei(mt5.src_embedding(), "src");
      if (!res.empty()) return res;
    }

    res = proto_parse_emb_wei(mt5.trg_embedding(), "trg");
    if (!res.empty()) return res;

    if (!only_decoder) {
      res = proto_parse_enc_wei(mt5);
      if (!res.empty()) return res;
    }

    res = proto_parse_dec_wei(mt5);
    if (!res.empty()) return res;
