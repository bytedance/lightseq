...
*/
template <>
float XLMRWeight<OperationType::FP32>::float2required(float value) {
  return value;
}
...
*/
template <>
__half XLMRWeight<OperationType::FP16>::float2required(float value) {
...
void XLMRWeight<OpType_>::proto_get_model_config(const XLMR &xlmr,
                                                 bool only_decoder) {
  _hidden_size = xlmr.trg_embedding().norm_scale_size();
  _inner_size = xlmr.decoder_stack()[0].ffn_first_kernel_size() / _hidden_size;
  _max_step = xlmr.trg_embedding().position_embedding_size() / _hidden_size;
  if (!only_decoder) {
    _src_vocab_size =
        xlmr.src_embedding().token_embedding_size() / _hidden_size;
  }
  _trg_vocab_size = xlmr.trg_embedding().token_embedding_size() / _hidden_size;
  if (!only_decoder) {
    _n_enc_layer = xlmr.encoder_stack_size();
  }
  _n_dec_layer = xlmr.decoder_stack_size();
  _head_num = xlmr.model_conf().head_num();
  ...
  _beam_size = xlmr.model_conf().beam_size();
  _extra_decode_length = xlmr.model_conf().extra_decode_length();
  _length_penalty = xlmr.model_conf().length_penalty();
  _padding_id = xlmr.model_conf().src_padding_id();
  _start_id = xlmr.model_conf().trg_start_id();
  _end_id = xlmr.model_conf().trg_end_id();
  if (_end_id == 0) {
    _end_id = _trg_vocab_size - 1;
  }
  _diverse_lambda = xlmr.model_conf().diverse_lambda();
  _sampling_method = xlmr.model_conf().sampling_method();
  if (_sampling_method == "") {
    _sampling_method = "beam_search";
  }
  _topk = xlmr.model_conf().topk();
  _topp = xlmr.model_conf().topp();
  _is_post_ln = xlmr.model_conf().is_post_ln();
  _no_scale_embedding = xlmr.model_conf().no_scale_embedding();
  _use_gelu = xlmr.model_conf().use_gelu();
}
...
*/
template <OperationType OpType_>
std::string XLMRWeight<OpType_>::proto_parse_emb_wei(
    const XLMREmbeddingLayer &layer, std::string source) {
...
*/
template <OperationType OpType_>
std::string XLMRWeight<OpType_>::proto_parse_enc_wei(const XLMR &xlmr) {
  std::vector<int> offset;
  std::vector<float> value;
  int idx = 0;

  for (auto enc_layer : xlmr.encoder_stack()) {
    offset.push_back(idx);
...
*/
template <OperationType OpType_>
std::string XLMRWeight<OpType_>::proto_parse_dec_wei(const XLMR &xlmr) {
  std::vector<int> offset;
  std::vector<float> value;
  int idx = 0;

  for (auto dec_layer : xlmr.decoder_stack()) {
    offset.push_back(idx);
...
*/
template <OperationType OpType_>
void XLMRWeight<OpType_>::hdf5_get_model_config(hid_t hdf5_file,
                                                bool only_decoder) {
...
*/
template <OperationType OpType_>
void XLMRWeight<OpType_>::hdf5_parse_emb_wei(hid_t hdf5_file,
                                             std::string source) {
  int vocab_size = (source == "src") ? _src_vocab_size : _trg_vocab_size;
  ...
  if (source == "src") {
    ...
    if (lang_emb_size / _hidden_size != trg_vocab_mask_size / _trg_vocab_size) {
...
*/
template <OperationType OpType_>
void XLMRWeight<OpType_>::hdf5_parse_enc_wei(hid_t hdf5_file) {
  size_t value_size =
...
*/
template <OperationType OpType_>
void XLMRWeight<OpType_>::hdf5_parse_dec_wei(hid_t hdf5_file) {
  size_t value_size =
      (_hidden_size * 2 + _hidden_size * _hidden_size * 3 + _hidden_size * 3 +
       _hidden_size * _hidden_size + _hidden_size * 3 +
...
*/
template <OperationType OpType_>
std::string XLMRWeight<OpType_>::initializing(std::string weight_path,
                                              bool only_decoder) {
  ...
  if (endswith(weight_path, ".pb")) {
    ...
    if (!xlmr.ParseFromIstream(&raw_input)) {
      return "Parse weights from [" + weight_path + "] failed.";
    }
    proto_get_model_config(xlmr, only_decoder);
    ...
    std::string res;
    if (!only_decoder) {
      res = proto_parse_emb_wei(xlmr.src_embedding(), "src");
      if (!res.empty()) return res;
    }

    res = proto_parse_emb_wei(xlmr.trg_embedding(), "trg");
    if (!res.empty()) return res;

    if (!only_decoder) {
      res = proto_parse_enc_wei(xlmr);
      if (!res.empty()) return res;
    }

    res = proto_parse_dec_wei(xlmr);
    if (!res.empty()) return res;
...
template class XLMRWeight<OperationType::FP16>;
template class XLMRWeight<OperationType::FP32>;
