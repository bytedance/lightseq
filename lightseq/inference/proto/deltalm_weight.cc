...
*/
template <>
float DeltalmWeight<OperationType::FP32>::float2required(float value) {
  return value;
}
...
*/
template <>
__half DeltalmWeight<OperationType::FP16>::float2required(float value) {
...
void DeltalmWeight<OpType_>::proto_get_model_config(
    const Deltalm &Deltalm, bool only_decoder) {
  _hidden_size = Deltalm.trg_embedding().norm_scale_size();
  _inner_size =
      Deltalm.decoder_stack()[0].ffn_first_kernel_size() / _hidden_size;
  _max_step =
      Deltalm.trg_embedding().position_embedding_size() / _hidden_size;
  if (!only_decoder) {
    _src_vocab_size =
        Deltalm.src_embedding().token_embedding_size() / _hidden_size;
  }
  _trg_vocab_size =
      Deltalm.trg_embedding().token_embedding_size() / _hidden_size;
  if (!only_decoder) {
    _n_enc_layer = Deltalm.encoder_stack_size();
  }
  _n_dec_layer = Deltalm.decoder_stack_size();
  _head_num = Deltalm.model_conf().head_num();
  ...
  _weight_per_dec_layer = 24;
  _beam_size = Deltalm.model_conf().beam_size();
  _extra_decode_length = Deltalm.model_conf().extra_decode_length();
  _length_penalty = Deltalm.model_conf().length_penalty();
  _padding_id = Deltalm.model_conf().src_padding_id();
  _start_id = Deltalm.model_conf().trg_start_id();
  _end_id = Deltalm.model_conf().trg_end_id();
  if (_end_id == 0) {
    _end_id = _trg_vocab_size - 1;
  }
  _diverse_lambda = Deltalm.model_conf().diverse_lambda();
  _sampling_method = Deltalm.model_conf().sampling_method();
  if (_sampling_method == "") {
    _sampling_method = "beam_search";
  }
  _topk = Deltalm.model_conf().topk();
  _topp = Deltalm.model_conf().topp();
  _is_post_ln = Deltalm.model_conf().is_post_ln();
  _no_scale_embedding = Deltalm.model_conf().no_scale_embedding();
  _use_gelu = Deltalm.model_conf().use_gelu();
  _multilg_type = Deltalm.model_conf().multilg_type();
}
...
*/
template <OperationType OpType_>
std::string DeltalmWeight<OpType_>::proto_parse_emb_wei(
    const DeltalmEmbeddingLayer &layer, std::string source) {
...
*/
template <OperationType OpType_>
std::string DeltalmWeight<OpType_>::proto_parse_enc_wei(
    const Deltalm &deltalm) {
  std::vector<int> offset;
  std::vector<float> value;
  int idx = 0;

  for (auto enc_layer : deltalm.encoder_stack()) {
    offset.push_back(idx);
...
*/
template <OperationType OpType_>
std::string DeltalmWeight<OpType_>::proto_parse_dec_wei(
    const Deltalm &deltalm) {
  std::vector<int> offset;
  std::vector<float> value;
  int idx = 0;

  for (auto dec_layer : deltalm.decoder_stack()) {
    offset.push_back(idx);
    ...
    // middle ffn wei
    offset.push_back(idx);
    if (dec_layer.middle_ffn_norm_scale_size() != _hidden_size)
      return "Wrong ffn_norm_scale_size !";
    for (float ele : dec_layer.middle_ffn_norm_scale()) value.push_back(ele);
    idx += _hidden_size;

    offset.push_back(idx);
    if (dec_layer.middle_ffn_norm_bias_size() != _hidden_size)
      return "Wrong ffn_norm_bias_size !";
    for (float ele : dec_layer.middle_ffn_norm_bias()) value.push_back(ele);
    idx += _hidden_size;

    offset.push_back(idx);
    if (dec_layer.middle_ffn_first_kernel_size() != _hidden_size * _inner_size)
      return "Wrong ffn_first_kernel_size !";
    for (float ele : dec_layer.middle_ffn_first_kernel()) value.push_back(ele);
    idx += _hidden_size * _inner_size;

    offset.push_back(idx);
    if (dec_layer.middle_ffn_first_bias_size() != _inner_size)
      return "Wrong ffn_first_bias_size !";
    for (float ele : dec_layer.middle_ffn_first_bias()) value.push_back(ele);
    idx += _inner_size;

    offset.push_back(idx);
    if (dec_layer.middle_ffn_second_kernel_size() != _hidden_size * _inner_size)
      return "Wrong ffn_second_kernel_size !";
    for (float ele : dec_layer.middle_ffn_second_kernel()) value.push_back(ele);
    idx += _hidden_size * _inner_size;

    offset.push_back(idx);
    if (dec_layer.middle_ffn_second_bias_size() != _hidden_size)
      return "Wrong ffn_second_bias_size !";
    for (float ele : dec_layer.middle_ffn_second_bias()) value.push_back(ele);
    idx += _hidden_size;
...
*/
template <OperationType OpType_>
void DeltalmWeight<OpType_>::hdf5_get_model_config(hid_t hdf5_file,
                                                       bool only_decoder) {
...
*/
template <OperationType OpType_>
void DeltalmWeight<OpType_>::hdf5_parse_emb_wei(hid_t hdf5_file,
                                                    std::string source) {
  int vocab_size = (source == "src") ? _src_vocab_size : _trg_vocab_size;
...
*/
template <OperationType OpType_>
void DeltalmWeight<OpType_>::hdf5_parse_enc_wei(hid_t hdf5_file) {
  size_t value_size =
...
*/
template <OperationType OpType_>
void DeltalmWeight<OpType_>::hdf5_parse_dec_wei(hid_t hdf5_file) {
  size_t value_size =
      (_hidden_size * 2 + _hidden_size * _hidden_size * 3 + _hidden_size * 3 +
       _hidden_size * _hidden_size + _hidden_size * 3 +
       _hidden_size * _hidden_size + _hidden_size +
       _hidden_size * _hidden_size + _hidden_size * 3 +
       (_hidden_size * _inner_size + _inner_size + _hidden_size * _inner_size + _hidden_size) * 2 + _hidden_size * 2 
       ) *
      _n_dec_layer;
  std::vector<int> offset;
  std::vector<float> value(value_size);
  ...
  for (int layer_id = 0; layer_id < _n_dec_layer; ++layer_id) {
    ...
    offset.push_back(idx);
    read_hdf5_dataset_data(
        hdf5_file, dataset_prefix + "/middle_ffn_norm_scale", H5T_NATIVE_FLOAT,
        value.data() + idx, [=](int size) { return size != _hidden_size; },
        "Wrong ffn_norm_scale_size !");
    idx += _hidden_size;
    ...
    offset.push_back(idx);
    read_hdf5_dataset_data(
        hdf5_file, dataset_prefix + "/middle_ffn_norm_bias", H5T_NATIVE_FLOAT,
        value.data() + idx, [=](int size) { return size != _hidden_size; },
        "Wrong ffn_norm_bias_size !");
    idx += _hidden_size;
    ...
    offset.push_back(idx);
    read_hdf5_dataset_data(
        hdf5_file, dataset_prefix + "/middle_ffn_first_kernel", H5T_NATIVE_FLOAT,
        value.data() + idx,
        [=](int size) { return size != _hidden_size * _inner_size; },
        "Wrong ffn_first_kernel_size !");
    idx += _hidden_size * _inner_size;
    ...
    offset.push_back(idx);
    read_hdf5_dataset_data(
        hdf5_file, dataset_prefix + "/middle_ffn_first_bias", H5T_NATIVE_FLOAT,
        value.data() + idx, [=](int size) { return size != _inner_size; },
        "Wrong ffn_first_bias_size !");
    idx += _inner_size;
    ...
    offset.push_back(idx);
    read_hdf5_dataset_data(
        hdf5_file, dataset_prefix + "/middle_ffn_second_kernel", H5T_NATIVE_FLOAT,
        value.data() + idx,
        [=](int size) { return size != _hidden_size * _inner_size; },
        "Wrong ffn_second_kernel_size !");
    idx += _hidden_size * _inner_size;
    ...
    offset.push_back(idx);
    read_hdf5_dataset_data(
        hdf5_file, dataset_prefix + "/middle_ffn_second_bias", H5T_NATIVE_FLOAT,
        value.data() + idx, [=](int size) { return size != _hidden_size; },
        "Wrong ffn_second_bias_size !");
    idx += _hidden_size;
...
*/
template <OperationType OpType_>
std::string DeltalmWeight<OpType_>::initializing(std::string weight_path,
                                                     bool only_decoder) {
  ...
  if (endswith(weight_path, ".pb")) {
    ...
    if (!Deltalm.ParseFromIstream(&raw_input)) {
      return "Parse weights from [" + weight_path + "] failed.";
    }
    proto_get_model_config(Deltalm, only_decoder);
    ...
    std::string res;
    if (!only_decoder) {
      res = proto_parse_emb_wei(Deltalm.src_embedding(), "src");
      if (!res.empty()) return res;
    }

    res = proto_parse_emb_wei(Deltalm.trg_embedding(), "trg");
    if (!res.empty()) return res;

    if (!only_decoder) {
      res = proto_parse_enc_wei(Deltalm);
      if (!res.empty()) return res;
    }

    res = proto_parse_dec_wei(Deltalm);
    if (!res.empty()) return res;
...
template class DeltalmWeight<OperationType::FP16>;
template class DeltalmWeight<OperationType::FP32>;
