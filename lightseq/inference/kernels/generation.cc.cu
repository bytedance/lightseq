...
*/
template <typename T>
__global__ void ker_process_logits(const int* enc_input_ids, const int* pad_mask,
  ...
  if (encoder_no_repeat_ngram_size > 0 &&
    ...
    for (int i = threadIdx.x; i <= enc_seq_len - 1 - n_match; i += blockDim.x) {
      if (pad_mask[flat_2dim(blockIdx.x, i + n_match, enc_seq_len)] > 0) {
