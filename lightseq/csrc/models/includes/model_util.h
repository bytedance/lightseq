...
template<typename T>
void refresh_cache(int grid_dim_x, int grid_dim_y, int block_dim, cudaStream_t stream, 
        const int* num_can_per_beam, const int* can_idx,
        Variable* caches_k, Variable* caches_v, Variable* caches_k_buf, Variable* caches_v_buf,
        size_t cache_block_size, int beam_size, int dim_per_head, int head_num,
    ...
    cuda::ker_refresh_cache_launcher<T>(
        ...
        (T*)caches_v_buf_ptr, cache_block_size, beam_size, dim_per_head,
