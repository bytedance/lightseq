...
*/
template <template OpType_>
class BertWeight {
 private:

  OpType_ float2required(float value);
  ...
  std::vector<const OpType_ *> _p_d_src_emb_wei;  // size: 4
  std::vector<const OpType_ *> _p_d_enc_wei;      // size: 12 * enc_layer_num

  // store the weights on gpu memory
  thrust::device_vector<OpType_> _d_src_emb_wei;
  thrust::device_vector<OpType_> _d_enc_wei;

 public:
  std::string initializing(std::string proto_path);

  const std::vector<const OpType_ *> &get_src_emb_wei() const {
    // {token_emb, pos_emb, norm_scale, norm_bias}
    return _p_d_src_emb_wei;
  }

  const std::vector<const OpType_ *> &get_enc_wei() const {
