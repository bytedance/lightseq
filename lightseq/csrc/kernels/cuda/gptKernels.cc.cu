...
__global__ void kernel_gpt_embedding(const T* token_emb, const T* pos_emb,
  ...
  if (token_id == padding_id) {
    ...
    float4 token_emb_val =  ((float4 *)token_emb)[token_id * hidden_dim + state_idx];
    float4 pos_emb_val = ((float4 *)pos_emb)[(seq_idx + step_offset) * hidden_dim + state_idx];
  ...
  ((float4 *)output)[idx] = output_val;
}


template <>
__global__ void kernel_gpt_embedding<__half>(const __half* token_emb, const __half* pos_emb,
                                     const int* token_ids, __half* output,
                                     int batch_size, int beam_size, int seq_len,
                                     int hidden_dim, int padding_id,
                                     int max_step, int step_offset) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
//   float4 output_val = ((float4 *)output)[idx];
//   if (token_id == padding_id) {
//     __half* output_h2 = (__half2 *)(&output_val);
// #pragma unroll
//     for(int i = 0; i < 4; i ++) {
...
void launch_gpt_embedding<float>(const float* token_emb, const float* pos_emb,
  ...
  int nele = (batch_size * beam_size * seq_len * hidden_dim) / 4;
...
template <>
void launch_gpt_embedding<__half>(const __half* token_emb, const __half* pos_emb,
  ...
  int nele = (batch_size * beam_size * seq_len * hidden_dim) / 8;
  int nblock = (nele + MAX_THREADS - 1) / MAX_THREADS;
...
template void launch_gpt_embedding<float>(const float* token_emb, const float* pos_emb,
                                 const int* tokens, float* output,
                                 int batch_size, int beam_size, int hidden_dim,
                                 int step_offset, int seq_len, int max_step,
                                 int padding_id, cudaStream_t stream) ;
...
template void launch_gpt_embedding<__half>(const __half* token_emb, const __half* pos_emb,
                                 const int* tokens, __half* output,
                                 int batch_size, int beam_size, int hidden_dim,
                                 int step_offset, int seq_len, int max_step,
                                 int padding_id, cudaStream_t stream);
