...
Variable::Variable(std::string name, size_t mx_size, size_t sizeof_value, size_t sizeof_grad): 
    Node(name), _mx_size(mx_size) {
    _value.reset(new Tensor(this->_name + "_value", mx_size * sizeof_value, true));
    // auto real_context_ptr = _context_ptr.lock();
...
Variable::Variable(std::string name, size_t mx_size,
           const T1* para_ptr, T2* grad_ptr)  // for parameter
      : Node(name), _mx_size(mx_size) {
    // auto real_context_ptr = _context_ptr.lock();
...
template Variable::Variable<int, int>(std::string name, size_t mx_size, const int* para_ptr, int* grad_ptr);
