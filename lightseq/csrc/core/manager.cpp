...
namespace lightseq {
void MemoryManager::update_tensor_life_idx(int unique_id, int node_idx, size_t size,
                              std::string name) {
    std::map<int, TensorUsage>::iterator iter =
        tensor_usages_.find(unique_id);
...
void MemoryManager::calculate_buffer_() {
    ...
    for (int idx = 0; idx < tensor_usages_vec.size(); idx++) {
      ...
      for (auto allocated_tensor : ordered_tensor_usages) {
        ...
        size_t min_last_op = std::min(cal_tensor_usage.last_idx,
                                      allocated_tensor_usage.last_idx);
        ...
        if (max_first_op <= min_last_op) {
          ...
          prev_offset = std::max(
              prev_offset, allocated_offset + allocated_tensor_usage.size);
        }
      }
      if (!best_offset_flag) {
        best_offset = prev_offset;
