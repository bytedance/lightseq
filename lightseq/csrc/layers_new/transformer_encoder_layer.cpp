#include "transformer_self_attention.h"

namespace lightseq {

template <typename T1, typename T2>
TransformerSelfAttention<T1, T2>::TransformerSelfAttention(
    ...
    const T1* para_ptr, T2* grad_ptr, int &offset): 
      Layer("TransformerSelfAttention"),
      _layer_id(layer_id),
...
template <typename T1, typename T2>
void TransformerSelfAttention<T1, T2>::operator()(Variable* inp, Variable* inp_mask) {
...
template <typename T1, typename T2>
void TransformerSelfAttention<T1, T2>::before_forward(int batch_size, int seq_len) {
