...
class LyrNormalizeLayerWeight {
 public:
  LyrNormalizeLayerWeight(int hidden_size)
  ...
  int load_params(const std::vector<const T*>& para_vec, int offset) {
    int size = 0;
    _gamma_ptr = (char*)para_vec[offset + size]; size ++;
    _betta_ptr = (char*)para_vec[offset + size]; size ++;

    return size;
  }
};

template int LyrNormalizeLayerWeight::load_para_and_grad(const float* para_ptr, float* grad_ptr);
template int LyrNormalizeLayerWeight::load_para_and_grad(const __half* para_ptr, __half* grad_ptr);

template int LyrNormalizeLayerWeight::load_params(const std::vector<const float*>& para_vec, int offset);
template int LyrNormalizeLayerWeight::load_params(const std::vector<const __half*>& para_vec, int offset);
...
class LyrNormalizeLayer : public Layer {
 ...
 public:
  LyrNormalizeLayer(LyrNormalizeLayerWeightPtr norm_wt, int max_batch_tokens, int hidden_size):
    Layer("LyrNormalizeLayer"),
    _lyr_norm_op(new LayerNormalizeOp<T1, T2>(max_batch_tokens, hidden_size)) {
        
        _norm_gamma = new Variable("layer_norm_gamma", norm_wt->_gamma_ptr, norm_wt->_grad_gamma_ptr);

        _norm_betta = new Variable("layer_norm_betta", norm_wt->_betta_ptr, norm_wt->_grad_betta_ptr);
  ...
  void before_forward(int batch_token_num) { _lyr_norm_op->before_forward(batch_token_num); }
