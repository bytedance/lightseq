...
SDPALayer<T1, T2>::SDPALayer(int max_batch_tokens, int max_seq_len,
  ...
  _attn_scores =
      new StridedBatchGemmOp<T1, T2>(max_batch_tokens * num_heads * max_seq_len,
                                     scale, T1(0.0), CUBLAS_OP_T, CUBLAS_OP_N);
  ...
  _attn_context = new StridedBatchGemmOp<T1, T2>(
      max_batch_tokens * num_heads * head_dim, T1(1.0), T1(0.0), CUBLAS_OP_N,
      CUBLAS_OP_N);
...
Variable* SDPALayer<T1, T2>::operator()(Variable* query, Variable* key,
  ...
  Variable* attn_context = (*_attn_context)(value, soft_out);
