...
template <typename T>
std::tuple<Variable*, Variable*, Variable*> BeamSearchTopkOp<T>::operator()(Variable* logits, Variable* logit_bias, Variable* seq_probs,
    Variable* seq_score, Variable* alive_seq) {

  Variable* can_idx = new Variable("can_idx",  _max_batch_size * _beam_size * _trg_vocab_size * sizeof(int));

  Variable* can_score = new Variable("can_score", _max_batch_size * _beam_size * _trg_vocab_size * sizeof(float));

  Variable* num_beam_can = new Variable("num_beam_can", (_max_batch_size * _beam_size + 1) * sizeof(int));
...
void BeamSearchTopkOp<T>::forward() {
  ...
  */
  ...
  select_beam_rough_topk_launcher(
      logits_ptr, logits_bias_ptr, seq_probs_ptr,
      seq_score_ptr, alive_seq_ptr, can_idx_ptr, can_score_ptr,
      num_beam_can_ptr, _trg_vocab_size, _max_step,
      _length_norm, _cur_step, _step_token_num,
  ...
  thrust::exclusive_scan(thrust::cuda::par.on(stream), num_beam_can_ptr + 1,
                         num_beam_can_ptr + 1 + _step_token_num, num_beam_can_ptr + 1);



  /* ---step 2. sort the candidate with their probability--- */
  CHECK_GPU_ERROR(cudaMemcpyAsync(&_host_can_num_batch, num_beam_can_ptr, sizeof(int),
  ...
  if (_diverse_lambda != 0) {
    ...
    ker_diverse_beam_search_launcher(can_score_ptr, can_idx_ptr, num_beam_can_ptr,
