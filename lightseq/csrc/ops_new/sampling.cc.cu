...
template<typename T>
SamplingOp<T>::SamplingOp(GenerateMethod gm, int max_batch_size, int max_step, int max_thread_per_block, 
...
template<typename T>
Variable* SamplingOp<T>::operator()(Variable* logits, Variable* logit_bias, Variable* token_ids) {
    set_parents({logits, logit_bias, token_ids});

    _out_token_ids = new Variable("out_token_ids", _max_batch_size * _max_step, g_dtype<int>());
...
void SamplingOp<T>::forward()  {
    ...
    if(_generate_method == GenerateMethod::Topk) {
        cuda::ker_topk_sample_launcher<T>(
            _batch_size, _seq_len, _max_step, _logits_seq_len, _max_thread_per_block,
            _stream, logits_ptr, logits_bias_ptr, inp_tokens_ptr, out_tokens_ptr,
            _trg_vocab_size, _topk, _p_d_unfinished, _p_d_curandstate, _eos_id);
    }
    else if(_generate_method == GenerateMethod::Topp) {
        cuda::ker_topp_sample_launcher<T>(
            _batch_size, _seq_len, _max_step, _logits_seq_len, _max_thread_per_block,
            _stream, logits_ptr, logits_bias_ptr, inp_tokens_ptr,
            out_tokens_ptr, _trg_vocab_size, _topp,
            _p_d_unfinished, _p_d_curandstate, _eos_id);
    }
#endif 
}
