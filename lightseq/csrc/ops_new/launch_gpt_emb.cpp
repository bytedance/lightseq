...
std::tuple<Variable*, Variable*, Variable*> LaunchGptEmbOp<T>::operator()(
  ...
  _result = new Variable("LaunchGptEmbOp_out", _max_batch_tokens * _hidden_dim,
                         g_dtype<T>());
  _pad_mask =
      new Variable("_pad_mask", _max_batch_tokens, g_dtype<T>());

  _left_pad_len = new Variable("_left_pad_len", _max_batch_size * _beam_size, g_dtype<int>(), cuda::DataType::kNotSupported, VariableType::RegressiveVariable);
...
void LaunchGptEmbOp<T>::forward() {
  ...
  if (_offset == 0) {
    cudaMemsetAsync(left_pad_len_ptr, 0, _batch_size * _beam_size * sizeof(int), _stream);
  }
  cuda::launch_gpt_embedding<T>(token_emb, pos_emb, inp_tokens, output_ptr, pad_mask_ptr, left_pad_len_ptr,
                                _batch_size, _beam_size, _hidden_dim, _offset,
                                _seq_len, _max_step, _pad_id, _stream);
#endif
}
