...
class T5SoftmaxOp : public Operator {
 private:
  int _nhead;
 ...
 public:
  T5SoftmaxOp(int max_batch_tokens, int max_seq_len, int nhead,
            bool mask_future = false)
      : Operator("T5SoftmaxOp"),
  ...
  void before_backward(int batchs, int from_len, int to_len) {
    throw std::runtime_error("T5SoftmaxOp does not have backward function");
  }
};
