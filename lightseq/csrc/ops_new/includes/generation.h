...
template <typename T>
class GenerationOp : public Operator {
 private:
  // inital
  int _max_batch_size;
  int _max_step;
  ...
  int _enc_seq_len;  // for logit process, will be removed to a logit_process_op
  int _encoder_no_repeat_ngram_size;  // for logit process, will be removed to a
                                      // logit_process_op
  int _no_repeat_ngram_size;          // for logit process, will be removed to a
                                      // logit_process_op
 ...
 public:
  GenerationOp(int nshared_dec_layer, int max_batch_size, int max_step,
  ...
  void before_forward(int batch_size, int cur_step, int enc_seq_len) {
    _batch_size = batch_size;
    _cur_step = cur_step;
    _step_token_num = batch_size * _beam_size;
    _enc_seq_len = enc_seq_len;
  }
