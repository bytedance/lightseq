...
Variable* StridedBatchGemmOp<T1, T2>::operator()(Variable* inpA, Variable* inpB) {
  Variable* result =
...
void StridedBatchGemmOp<T1, T2>::forward() {
  ...
  cublas_strided_batched_gemm(
      handle, _m, _n, _k, &_alpha, &_beta,
      _buffer_a, _buffer_b, output, _op_A, _op_B, stride_a,
      stride_b, stride_c, _batch_heads, cublasGemmAlgo_t(_gemm_algos[0]));
}
...
void StridedBatchGemmOp<T1, T2>::backward() {
  ...
  cublasOperation_t op_b =
      (_op_B == CUBLAS_OP_T ? CUBLAS_OP_N : CUBLAS_OP_T);
  ...
  // Calculate d_B.
  cublas_strided_batched_gemm(
      handle, _k, _n, _m, &_alpha, &_beta,
      _buffer_a, d_output, inpGradB, op_a, CUBLAS_OP_N, stride_a, stride_b,
      stride_c, _batch_heads, cublasGemmAlgo_t(_gemm_algos[2]));
}
