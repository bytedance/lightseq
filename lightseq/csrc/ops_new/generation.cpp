...
GenerationOp<T>::GenerationOp(int nshared_dec_layer, int max_batch_size,
...
template <typename T>
std::tuple<Variable*, Variable*> GenerationOp<T>::operator()(
    Variable* logits, Variable* logit_bias, Variable* alive_seq,
    Variable* caches_k, Variable* caches_v, Variable* enc_input_ids,
    Variable* enc_pad_mask) {
  set_parents({logits, logit_bias, alive_seq, caches_k, caches_v, enc_input_ids,
               enc_pad_mask});
...
void GenerationOp<T>::forward() {
  ...
  int* enc_input_ids_ptr = (int*)parent(5)->value();
  T* enc_pad_mask_ptr = (T*)parent(6)->value();
  ...
  */
  ...
  launch_process_logits<T>(enc_input_ids_ptr, enc_pad_mask_ptr, alive_seq_ptr,
                           logits_ptr, _enc_seq_len, _cur_step, _max_step,
                           _encoder_no_repeat_ngram_size, _no_repeat_ngram_size,
                           _trg_vocab_size, _batch_size, _beam_size, stream);
...
template class GenerationOp<float>;
template class GenerationOp<__half>;
