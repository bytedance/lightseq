...
template <typename T>
void StridedBatchGemm<T>::Forward(int bsz, T *output, const T *_buffer_a, const T *_buffer_b,
...
template <typename T>
void StridedBatchGemm<T>::Backward(int bsz, const T *d_output, const T *_buffer_a,
                const T *_buffer_b, cublasHandle_t handle,
                T *inpGradA, T *inpGradB) {
...
template <typename T>
inline void StridedBatchGemm<T>::SetConfig(int m, int n, int k) { _config.SetConfig(m, n, k); }
