...
TransformerDecoderLayer<T>::TransformerDecoderLayer(
    ...
    : _layer_id(layer_id),
      ...
      _encdec_q_linear_v4(typename FeedForwardV4<T>::Config()),
      _encdec_kv_linear(
      ...
      _encdec_kv_linear_v4(typename FeedForwardV4<T>::Config()),
      ...
      _encdec_attn_out_linear_v4(typename FeedForwardV4<T>::Config()),
...
void TransformerDecoderLayer<T>::self_attn_layer_fw(const T *input_ptr,
  ...
  _attn_scores.Forward(batch_heads, _soft_out_ptr, k_tf_ptr, q_tf_ptr,
                       _cublasHandle);
  // _attn_scores_v2.Forward(k_tf_ptr, q_tf_ptr, _soft_out_ptr,
  //                         _cublasLtHandle, _stream);
  // Softmax + Mask
  ...
  _attn_context.Forward(batch_heads, buffer, v_tf_ptr, _attn_score_ptr,
                        _cublasHandle);
  // _attn_context_v2.Forward(v_tf_ptr, _attn_score_ptr, buffer,
  //                          _cublasLtHandle, _stream);
  ...
  //                             _cublasLtHandle, _stream);
  //   _attn_out_linear_v3.Forward(_quant_attn_ow_ptr, _attn_output_ptr,
  //   output_ptr,
  //                               _shared_ffn_input_ptr,
  //                               _shared_ffn_output_ptr, _cublasLtHandle,
  //                               _stream);
  _attn_out_linear_v4.Forward(_quant_attn_ow_ptr, _attn_output_ptr, output_ptr,
                              _shared_ffn_input_ptr, _shared_ffn_output_ptr,
                              _cublasHandle, _stream);
...
void TransformerDecoderLayer<T>::encdec_kv_fw(const T *enc_output_ptr) {
  ...
  //       _quant_encdec_attn_kvw_ptr, enc_output_ptr,
  //       _shared_grad_encdec_kv_ptr, _shared_ffn_input_ptr,
  //       _shared_ffn_output_ptr, _cublasLtHandle, _stream);
...
void TransformerDecoderLayer<T>::encdec_attn_layer_fw(const T *input_ptr,
  ...
  //                             _cublasLtHandle, _stream);
  //   _encdec_q_linear_v3.Forward(_quant_encdec_attn_qw_ptr, _gemmQ_inp_ptr,
  //   buffer,
  //                               _shared_ffn_input_ptr,
  //                               _shared_ffn_output_ptr, _cublasLtHandle,
  //                               _stream);
  ...
  _encdec_attn_scores_v2.Forward(encdec_k_ptr, _encdec_q_ptr,
                                 _encdec_soft_out_ptr, _cublasLtHandle,
                                 _stream);
  ...
  //       _shared_ffn_input_ptr, _shared_ffn_output_ptr, _cublasLtHandle,
  //       _stream);
...
template <typename T>
int8_t *TransformerDecoderLayer<T>::_shared_ffn_input_ptr = nullptr;
template <typename T>
int32_t *TransformerDecoderLayer<T>::_shared_ffn_output_ptr = nullptr;
