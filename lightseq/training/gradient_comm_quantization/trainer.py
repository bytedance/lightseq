...
class Trainer(object):
    ...
    def model(self):
        def encode_and_decode(
            state: QuantizeGradientState, bucket: dist.GradBucket
            ...
            quantizer = QuantizeGradient(
                world_size=world_size, hidden_size=hidden_size, bucket=bucket
            )
            scale_rate_list = quantizer.get_scale_rate_list()
        ...
        if self._wrapped_model is None:
            ...
            if isinstance(self._wrapped_model, DistributedDataParallel):
                if dist.get_rank() == 0:
                    print(
                        "self.args.encoder_embed_dim: {}".format(
                            self.args.encoder_embed_dim
                        )
                    )
                state = QuantizeGradientState(
                ...
                logger.info("############ register_common_hook done ###########")
    ...
    def _build_optimizer(self):
        ...
        # print('original_params length: {}'.format(len(params)))
        # for p in params:
        #     print(p.shape)
        # print('#################################################')
...
class QuantizeGradient(object):
    """Main class for bucket gridents splitting."""

    def __init__(
        self, world_size, hidden_size=1024, bucket=None, quantize_bits=8, quantile_value=0.99
    ):
        ...
        self.min_int8 = -self.max_int8
        self.new_max_int8 = self.max_int8.div(self.world_size, rounding_mode="trunc")
        self.new_min_int8 = self.min_int8.div(self.world_size, rounding_mode="trunc")

        self.original_gradient = self.bucket.buffer()
        self.bucket_size = self.original_gradient.numel()
        self.scale_rate_list = None
        self.encoded_gradient_whole_bucket = None

    def get_scale_rate_list(self):
        ...
        self.scale_rate_list = max_abs_bucket.div(self.max_int8)
        return self.scale_rate_list

    def encode_bucket(self):
        compressed_gradient = self.original_gradient.div(
            self.scale_rate_list * self.world_size
        )
        self.encoded_gradient_whole_bucket = (
            compressed_gradient.clamp_(min=self.new_min_int8, max=self.new_max_int8)
            .round_()
            .to(torch.int8)
        )
        return self.encoded_gradient_whole_bucket

    def decode_bucket(self):
        decompressed_gradient = self.bucket.buffer()
        decompressed_gradient.copy_(
            (
                self.encoded_gradient_whole_bucket.to(torch.float16)
                * self.scale_rate_list
            ).view(-1)[: self.bucket_size]
        )

        return decompressed_gradient


class QuantizeGradientState(object):
    """
    Prepare the state for gradient quantization.
    """

    def __init__(self, process_group, hidden_size=1024):

        self.process_group = process_group
        self.hidden_size = hidden_size
        logger.info(
            "############################## initialize QuantizedGradientState done ###########################"
        )
