...
void Encoder<OpType_>::run_one_infer(int batch_size, int batch_seq_len) {
  ...
  for(int i=0; i < _batch_size; i++) { // batch_id
      for(int j=0; j < _batch_token_num; j++) { // token_id
          ...
          print_vec(_p_d_output + i * _batch_seq_len * _tw._hidden_size + j *
			  _tw._hidden_size, "encoder_output", _tw._dim_per_head);
      }
 } //not normal
#endif
  return;
...
template <OperationType OpType_> void Encoder<OpType_>::self_attention() {
  ...
  ker_correlation_softmax_encself_launcher<_DataType>(_batch_size, 
  ...
  CHECK_GPU_ERROR(cublasGemmEx(_hd,
    ...
    &_fone,
    _p_d_output, _CType, _tw._hidden_size,
    _computeType,
    CUBLAS_GEMM_DEFAULT_TENSOR_OP));
  return;
}
...
template <OperationType OpType_> void Encoder<OpType_>::ffn_add_norm() {
  ...
  ker_bias_relu_launcher<_DataType>(_batch_token_num, _max_thread_per_block,
