...
void GptWeight<OpType_>::get_model_config(
    const Gpt &gpt) {
  _hidden_size = gpt.src_embedding().norm_scale_size();
  _inner_size =
      gpt.encoder_stack()[0].ffn_first_kernel_size() / _hidden_size;
  _max_step =
      gpt.src_embedding().position_embedding_size() / _hidden_size;
  _src_vocab_size =
      gpt.src_embedding().token_embedding_size() / _hidden_size;
...
template <OperationType OpType_>
std::string
GptWeight<OpType_>::parse_emb_wei(const EmbeddingLayer &layer) {
  ...
  std::cout << "finish initializing emb_wei from host to device"
            << std::endl;
  return "";
}
...
template <OperationType OpType_>
std::string
GptWeight<OpType_>::parse_enc_wei(const Gpt &gpt) {
  std::vector<int> offset;
  std::vector<float> value;
  int idx = 0;
...
std::string GptWeight<OpType_>::initializing(std::string proto_path) {
  ...
  if (!gpt.ParseFromZeroCopyStream(raw_input)) {
    delete raw_input;
    close(fd);
