...
INVERSE_SQUARE_ROOT_LR = "InverseSquareRootLR"
VALID_LR_SCHEDULES = [
    LR_RANGE_TEST,
    ONE_CYCLE,
    WARMUP_LR,
    WARMUP_DECAY_LR,
    INVERSE_SQUARE_ROOT_LR,
]
...
class InverseSquareRootLR(WarmupLR):
    def __init__(
        self,
        optimizer: Optimizer,
        warmup_min_lr: float = 0.0,
        ...
        super(InverseSquareRootLR, self).__init__(
            optimizer,
            warmup_min_lr,
            warmup_max_lr,
            warmup_num_steps,
    ...
    def get_lr(self):
        ...
        if self.last_batch_iteration < self.warmup_num_steps:
            return [
                s + (e - s) / self.warmup_num_steps * (self.last_batch_iteration + 1)
                for s, e in zip(self.min_lrs, self.max_lrs)
            ]
        return [
            e * self.warmup_num_steps ** 0.5 * (self.last_batch_iteration + 1) ** -0.5
            for e in self.max_lrs
        ]
